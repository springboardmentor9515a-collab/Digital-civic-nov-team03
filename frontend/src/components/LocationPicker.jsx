...
export default function LocationPicker({ value = null, onChange }) {
  ...
  async function handleMapConfirm() {
    ...
    try {
      ...
      setAddress(geo.display_name || '')
      setShowMap(false)
      onChange && onChange({ lat: pos.lat, lon: pos.lon, address: geo.display_name || '' })
    } catch (e) {
        ...
        <div className="map-modal">
          <div className="map-inner">
            <div className="map-header">
              <button className="btn small" onClick={() => setShowMap(false)}>Close</button>
              <span>Click on the map to place marker (drag to adjust)</span>
              <button className="btn small primary" onClick={handleMapConfirm}>Confirm</button>
            </div>
